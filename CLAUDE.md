...
- **Standard hardware autocomplete.** Inventory and BOM "name" inputs autocomplete from a curated list of common hardware (`M3x6 SHCS`, `M3x12 BHCS`, `Heatset M3`, etc.) so two projects don't end up with `M3 12mm SHCS` vs `M3x12 SHCS` typed slightly differently. Implementation: static `static/hardware-presets.json` + `<datalist>` on the inputs; picking a preset can prefill the unit (`each` for fasteners, `m` for wire).
